from model import LSTM,Linear_attention,GRU,RNN
import torch
...
def iteration(data_loader,device,model,cls,optim,train=True,norm=False,ratio=False):
    if train:
        model.train()
        cls.train()
...
if __name__ == '__main__':
    ...
    if args.model == "lstm":
        ...
        print("No such model!")
        exit(-1)
    cls = Linear_attention(output_dims=2).to(device)
    ...
    while True:
        j += 1
        loss, mean, std = iteration(train_loader, device, model, cls, optim, train=True, norm=args.norm, ratio=args.ratio)
        log = "Epoch {:} | Train Loss {:06f}, Train Mean {:06f}, Train Std {:06f} | ".format(j, loss, mean, std)
        print(log)
        with open("log.txt", 'a') as file:
            file.write(log)
        loss, mean, std = iteration(test_loader, device, model, cls, optim, train=False, norm=args.norm, ratio=args.ratio)

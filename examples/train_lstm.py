...
if __name__ == '__main__':
    ...
    while True:
        j += 1
        loss, last_loss = iteration(train_loader, device, model, cls, optim, train=True, norm=args.norm, prediction_len=args.prediction_len)
        log = "Epoch {:}, Train Loss {:06f}, Train Last Loss {:06f}".format(j, loss, last_loss)
        print(log)
        with open("LSTM.txt", 'a') as file:
            file.write(log)
        loss, last_loss = iteration(test_loader, device, model, cls, optim, train=False, norm=args.norm, prediction_len=args.prediction_len)
        ...
        if loss < best_loss:
            best_loss = loss
            loss_epoch = 0
            torch.save(cls.state_dict(), "LSTM_cls.pth")

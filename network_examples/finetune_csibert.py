from model import CSIBERT,Linear_attention
import torch
...
def iteration(data_loader,device,model,cls,optim,train=True):
    if train:
        model.train()
        cls.train()
...
if __name__ == '__main__':
    ...
    cls = Linear_attention(input_dims=128, output_dims=2).to(device)
    ...
    while True:
        j += 1
        loss, mean, std = iteration(train_loader, device, model, cls, optim, train=True)
        log = "Epoch {:} | Train Loss {:06f}, Train Mean {:06f}, Train Std {:06f} | ".format(j, loss, mean, std)
        print(log)
        with open("CSIBERT.txt", 'a') as file:
            file.write(log)
        loss, mean, std = iteration(test_loader, device, model, cls, optim, train=False)
